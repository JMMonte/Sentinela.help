...
export type CloudCoverOverlayConfig = {
  enabled: boolean;
};

export type CloudCoverOverlayState = {
  isAvailable: boolean;
  enabled: boolean;
...
export function useCloudCoverOverlay(
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const gridData = await fetchCloudCoverData();
      setData(gridData);
      setLastUpdated(new Date());
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to fetch cloud cover");
    } finally {
      setIsLoading(false);
    }
  }, [isAvailable]);
