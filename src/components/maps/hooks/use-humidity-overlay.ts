...
export type HumidityOverlayConfig = {
  enabled: boolean;
};

export type HumidityOverlayState = {
  isAvailable: boolean;
  enabled: boolean;
...
export function useHumidityOverlay(
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const gridData = await fetchHumidityData();
      setData(gridData);
      setLastUpdated(new Date());
    } catch (e) {
      setError(e instanceof Error ? e.message : "Failed to fetch humidity");
    } finally {
      setIsLoading(false);
    }
  }, [isAvailable]);
