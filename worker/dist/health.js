...
export function startHealthServer(port, scheduler) {
    startTime = Date.now();
    server = createServer(async (req, res) => {
        ...
        if (req.url === "/health" || req.url === "/") {
            try {
                ...
                const httpCode = status.status === "healthy" ? 200 : status.status === "degraded" ? 200 : 500;
...
async function getHealthStatus(scheduler) {
