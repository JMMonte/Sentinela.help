...
export class SeismicCollector extends MultiKeyCollector {
    ...
    async collect() {
        ...
        const results = await Promise.allSettled(Object.entries(USGS_FEEDS).map(async ([feedKey, url]) => {
        ...
        for (const result of results) {
            if (result.status === "fulfilled") {
                ...
                await this.storeToKey(`kaos:seismic:${feedKey}`, data, this.config.ttlSeconds);
