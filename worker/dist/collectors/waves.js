...
export class WavesCollector extends BaseCollector {
    ...
    async collect() {
        ...
        const response = await fetchWithRetry(url, { headers: { Accept: "application/json" } }, { timeoutMs: 120000, retries: 2 });
        ...
        const heightData = new Array(ny * nx).fill(NaN);
        const periodData = new Array(ny * nx).fill(NaN);
        const directionData = new Array(ny * nx).fill(NaN);
        let timeStr = "";
        // Fill the grid
        for (const row of rows) {
            const lat = row[latIdx];
