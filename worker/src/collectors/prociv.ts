...
export class ProcivCollector extends BaseCollector {
  ...
  protected async collect(): Promise<ProcivData> {
    ...
    const byId = new Map<string, FogosIncident>();
  ...
  private async fetchEndpoint(url: string): Promise<FogosResponse> {
    try {
      ...
      return (await response.json()) as FogosResponse;
    } catch {
