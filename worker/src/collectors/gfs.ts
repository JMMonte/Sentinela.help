...
function extractGfsField(messages: GribMessage[], category: number, parameter: number): any | null {
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function buildGridData(field: any, name: string, unit: string, transform?: (v: number) => number): GfsGridData {
  ...
  const dstData: number[] = new Array(dstNx * dstNy);
 ...
 */
function calculateUvIndex(ozoneDU: number, solarZenithAngle: number): number {
  if (solarZenithAngle >= 90) return 0;
...
export class GfsCollector extends MultiKeyCollector {
  ...
  protected async collect(): Promise<null> {
  ...
  private async collectTemperature(): Promise<void> {
    try {
      ...
      if (!field) throw new Error("Temperature field not found");
  ...
  private async collectHumidity(): Promise<void> {
    try {
      ...
      if (!field) throw new Error("Humidity field not found");
  ...
  private async collectPrecipitation(): Promise<void> {
    try {
      ...
      const convertedData = field.data.map((v: number) => v * 3600);
  ...
  private async collectCloudCover(): Promise<void> {
    try {
      ...
      if (!field) throw new Error("Cloud cover field not found");
  ...
  private async collectCape(): Promise<void> {
    try {
      ...
      if (!field) throw new Error("CAPE field not found");
  ...
  private async collectWind(): Promise<void> {
    try {
      ...
      if (!uField || !vField) throw new Error("Wind U/V fields not found");
      ...
      const downsampleData = (srcData: number[]): number[] => {
        const dstData: number[] = new Array(dstNx * dstNy);
        for (let dstY = 0; dstY < dstNy; dstY++) {
          const srcY = dstY * 2;
          for (let dstX = 0; dstX < dstNx; dstX++) {
  ...
  private async collectUvIndex(): Promise<void> {
    try {
      const url = buildGfsUrl([
      ...
      if (!ozoneField) throw new Error("Ozone field not found");
