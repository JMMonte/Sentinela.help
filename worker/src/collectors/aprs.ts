...
export class AprsCollector extends WebSocketCollector {
   ...
   */
  private parseAprsPacket(packet: string): AprsStation | null {
    try {
      ...
      if (!callsign || callsign.length > 9) return null;
   ...
   */
  private parsePosition(data: string): {
    latitude: number;
    longitude: number;
    symbol: string;
    symbolTable: string;
   ...
   */
  private parseUncompressedPosition(data: string): {
    latitude: number;
    longitude: number;
    symbol: string;
    symbolTable: string;
    ...
    try {
      ...
      if (latDir !== "N" && latDir !== "S") return null;
      ...
      if (lonDir !== "E" && lonDir !== "W") return null;
      ...
      let comment: string | null = rest.trim() || null;
   ...
   */
  private parseCompressedPosition(data: string): {
    latitude: number;
    longitude: number;
    symbol: string;
    symbolTable: string;
